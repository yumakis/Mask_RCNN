...
Usage: import the module (see Jupyter notebooks for examples), or run from
    ...
    python3 mica.py splash --weights=last --video=<URL or path to file>
"""
...
class MicaDataset(utils.Dataset):

    def load_mica(self, dataset_dir, subset):
...
def train(model):
    """Train the model."""
    # Training dataset.
    dataset_train = MicaDataset()
    dataset_train.load_mica(args.dataset, "train")
    dataset_train.prepare()

    # Validation dataset
    dataset_val = MicaDataset()
    dataset_val.load_mica(args.dataset, "val")
    dataset_val.prepare()
